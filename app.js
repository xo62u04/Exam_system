...
function showPage(name) {
  ...
  const pageMap = {
    'home': 'page-home',
    's1-notes': 'page-s1-notes',
    's2-notes': 'page-s2-notes',
    'quiz': 'page-quiz',
  };
  ...
  const navMap = {
    'home': 0, 's1-notes': 1, 's2-notes': 2, 'quiz': 3,
  };
  const btns = document.querySelectorAll('.nav-btn');
  if (navMap[name] !== undefined) btns[navMap[name]].classList.add('active');

  // 初始化筆記內容（只建一次）
  if (name === 's1-notes' && !document.querySelector('#s1-content .section-accordion')) {
    renderNotes('s1-content', S1_NOTES, 'blue-h');
  }
  if (name === 's2-notes' && !document.querySelector('#s2-content .section-accordion')) {
    renderNotes('s2-content', S2_NOTES, 'green-h');
  }
...
let quizState = {
  ...
  selected: null,
  answered: false,
  results: [],   // { correct: bool, selectedOpt, correctOpt }
  mode: '',
};
...
function startQuiz(mode) {
  ...
  let questions;
  if (mode === 's1') {
    questions = shuffle([...S1_QUESTIONS]);
  } else if (mode === 's2') {
    questions = shuffle([...S2_QUESTIONS]);
  } else {
    questions = shuffle([...S1_QUESTIONS, ...S2_QUESTIONS]);
  }
...
function checkAnswer() {
  ...
  document.querySelectorAll('.option-btn').forEach(btn => {
    ...
    } else if (btn.dataset.opt === quizState.selected && !correct) {
      btn.classList.remove('selected');
...
function finishQuiz() {
  ...
  breakdown.innerHTML = `
    <span class="rb-item rb-correct">✅ 答對 ${correct} 題</span>
    <span class="rb-item rb-wrong">❌ 答錯 ${wrong} 題</span>
    <span class="rb-item rb-total">📊 總分 ${score} 分</span>
  `;
}
...
function reviewAnswers() {
  ...
  quizState.results.forEach((r, i) => {
    ...
    div.innerHTML = `
      <div class="review-qnum">第 ${i + 1} 題　${q.subject}　${r.correct ? '✅ 答對' : '❌ 答錯'}</div>
      <div class="review-q">${escHtml(q.q)}</div>
      <div class="review-opts">${optsHtml}</div>
      <div class="review-exp">📖 ${escHtml(q.exp)}</div>
    `;
